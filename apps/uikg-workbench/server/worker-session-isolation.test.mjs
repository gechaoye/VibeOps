...
test('不同标签页的 Worker 会话可并发运行并独立取消与查询断点', async () => {
  ...
  const server = {
    ...
    async runWorkerModel({ signal, prompt }) {
  ...
  const startWorker = (workspaceSessionId, pageContext) => fetch(`${baseUrl}/workbench/api/workers/a/stream`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ frameId: 'sha256:test', pageContext, workspaceSessionId }),
  });
  const cancelWorker = (workspaceSessionId) => fetch(`${baseUrl}/workbench/api/workers/a/cancel`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  try {
    const [firstResponse, secondResponse] = await Promise.all([
      startWorker('tab-one', '页面一'),
      startWorker('tab-two', '页面二'),
    ]);
    assert.equal(firstResponse.status, 200);
    assert.equal(secondResponse.status, 200);
    while (startedSessions.size < 2) await new Promise((resolve) => setTimeout(resolve, 5));

    assert.deepEqual(await cancelWorker('tab-one'), { cancelled: true });
    assert.deepEqual(await cancelWorker('unknown-tab'), { cancelled: false });
    assert.deepEqual(await cancelWorker('tab-two'), { cancelled: true });
    await Promise.all([firstResponse.text(), secondResponse.text()]);

    const firstSession = await (await fetch(`${baseUrl}/workbench/api/workers/a/session?workspaceSessionId=tab-one`)).json();
    const secondSession = await (await fetch(`${baseUrl}/workbench/api/workers/a/session?workspaceSessionId=tab-two`)).json();
    const unknownSession = await (await fetch(`${baseUrl}/workbench/api/workers/a/session?workspaceSessionId=unknown-tab`)).json();
