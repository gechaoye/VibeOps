...
test('Worker B 手动中断后从输出断点继续', async () => {
  ...
  const result = {
    ...
    elements: [{
      candidateKey: 'worker-b.title', label: '消息', visualDescription: '顶部标题', controlType: 'label', interactive: false,
  ...
  const modelServer = createServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      assert.match(payload.messages[1].content[0].text, /worker-b\.title/);
  ...
  try {
    ...
    draft = mergeWorkerIntoDraft(draft, {
      ...
      page: { name: '消息', surfaceType: 'page', stateSummary: '消息页', scrollableRegions: [] },
      elements: [{
        candidateKey: 'worker-a.title', label: '消息', visualDescription: '标题', controlType: 'label', interactive: false,
        enabled: true, state: null, approximateRegion: { x: 0.1, y: 0.05, width: 0.3, height: 0.05 }, geometryKind: 'boundary', geometryConfidence: 0.9,
        meaning: { status: 'known', description: '页面标题', evidence: { visibleTexts: ['消息'], visibleIcons: [], visibleStates: [], visualCues: [], userContext: null, unclassified: [] } },
    ...
    const cancel = await fetch(`${baseUrl}/workbench/api/workers/b/cancel`, { method: 'POST' });
    ...
    const resumed = await fetch(`${baseUrl}/workbench/api/workers/b/resume/stream`, {
    ...
    assert.equal(completed.data.workerResult.elements[0].candidateKey, 'worker-b.title');
