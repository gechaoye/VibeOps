...
export const WORKER_ERROR_RETRY_LIMIT = 5;
...
export function recoverWorkerCheckpointFromStream(accumulated) {
...
export function summarizeWorkerCheckpoint(result) {
...
export function mergeWorkerContinuation(baseResult, continuation) {
...
export async function runResumableWorker({
  initialPrompt,
  initialResult,
  initialFallback,
  callWorker,
  ...
  while (true) {
    ...
    const checkpoint = summarizeWorkerCheckpoint(rawResult || initialFallback || {});
    const prompt = continuation
      ? buildContinuationPrompt(checkpoint, consecutiveFailures)
      : initialPrompt;
    try {
      const result = await callWorker(prompt, {
        signal,
        attempt: consecutiveFailures,
        continuation,
      });
      rawResult = continuation
        ? mergeWorkerContinuation(rawResult || initialFallback, result)
        : result;
      ...
      continuation = summarizeWorkerCheckpoint(rawResult).completedCandidates.length > 0;
      onRetry({
        attempt: consecutiveFailures,
        retryLimit,
        checkpoint: summarizeWorkerCheckpoint(rawResult),
        error: message,
      });
    }
  }
