...
function workerElement(candidateKey = 'header.title') {
  return {
    candidateKey,
    label: '消息',
...
test('模型错误重试从已收到的断点继续并最终合并草稿', async () => {
  ...
  const agent = {
    ...
    async runWorkerModel({ prompt, onChunk }) {
      modelCalls += 1;
      prompts.push(prompt);
      if (modelCalls === 1) {
        const checkpoint = { frameId: frozenFrameId, page: { name: '消息', surfaceType: 'page', stateSummary: '消息页', scrollableRegions: [] }, elements: [workerElement()] };
  ...
  await registerWorkbenchRoutes({
    server: { app, agent, runWorkerModel: agent.runWorkerModel, getSessionState: () => null },
    store,
    graphWorkflow: {},
