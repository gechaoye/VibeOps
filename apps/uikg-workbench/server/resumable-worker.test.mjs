...
import {
  mergeWorkerContinuation,
  recoverWorkerCheckpointFromStream,
  runResumableWorker,
  WORKER_ERROR_RETRY_LIMIT,
  summarizeWorkerCheckpoint,
} from './resumable-worker.mjs';
...
function completeWorker(elements) {
  return {
...
test('Worker A 续写按 candidateKey 合并元素并去重关系与动作', () => {
  const base = {
    frameId: 'sha256:frame',
    page: { name: '消息' },
    elements: [element('header', 0)],
  };
  const merged = mergeWorkerContinuation(base, {
  ...
  assert.equal(summarizeWorkerCheckpoint(merged).coveredBottom, 0.30000000000000004);
});

test('Worker A 流错误后仅恢复结构完整的候选作为断点', () => {
  const complete = {
    ...element('header', 0),
  ...
  const recovered = recoverWorkerCheckpointFromStream(stream);
...
test('正常结束但结构未完成时不自动重试', async () => {
  let calls = 0;
  const result = await runResumableWorker({
...
test('模型错误重试从断点续写，并在收到内容后重置计数', async () => {
  ...
  const result = await runResumableWorker({
    initialPrompt: 'initial',
    callWorker: async (prompt) => {
      prompts.push(prompt);
      calls += 1;
      if (calls === 1) {
        const error = new Error('连接断开 1');
        error.workerCheckpoint = partial;
        error.receivedContent = true;
        throw error;
      }
      if (calls === 2) {
        const error = new Error('连接断开 2');
        error.workerCheckpoint = { elements: [element('list-item-1', 0.2)] };
        error.receivedContent = true;
        throw error;
      }
      if (calls === 3) throw new Error('连接断开 3');
      return { ...completeWorker([element('footer', 0.9)]), done: true };
    },
...
test('超过 5 次模型错误后返回最后一次错误，不使用 30 秒超时', async () => {
  ...
  assert.equal(WORKER_ERROR_RETRY_LIMIT, 5);
...
test('首次模型错误且没有可解析输出时从画面起点重试', async () => {
  ...
  const result = await runResumableWorker({
    ...
    callWorker: async () => {
      ...
      return { ...completeWorker([element('header', 0)]), done: true };
    },
    buildContinuationPrompt: () => 'continue',
