...
const RECOGNITION_ELEMENT_SHAPE = `{candidateKey: string, label: string|null, visualDescription: string, displayCondition: string, elementType: ${ELEMENT_TYPE_UNION}, interactive: boolean, enabled: boolean|null, state: string|null, approximateRegion: {x:number,y:number,width:number,height:number}, geometryKind:"boundary"|"tap-target"|"approximate", geometryConfidence:number, meaning:{status:"known"|"candidate"|"unknown",description:string|null,evidence:{visibleTexts:string[],visibleIcons:string[],visibleStates:string[],visualCues:string[],userContext:string|null,unclassified:{type:string,detail:string|null}[]}}, dynamicContent:boolean, abstraction:null|{kind:"repeated-template"|"dynamic-template",templateKey:string,instanceCount:number,fields:{key:string,label:string,elementType:string,description:string,displayCondition:string,capabilities:string[],interactionBoundary:string,actionEffects:{action:string,effect:string}[],parentId:string|null,required:boolean,instanceRegions:{x:number,y:number,width:number,height:number}[]}[],instanceRegions:{x:number,y:number,width:number,height:number}[],bboxStyle:"abstract"}, riskSignals:string[], confidence:number}`;
...
function baseRuntimeStructureContext(runtimeStructure) {
  ...
  return `以下结构与截图来自同一次冻结。hierarchy.root 是 Android 运行时节点树，bounds 的坐标空间由 coordinateSpace、origin 和 viewport 明确声明。运行时节点具有 bounds 时，必须优先按 bounds/viewport 计算 approximateRegion，不得再次按 DPR 缩放或自行目测替代；只有找不到对应运行时节点时才使用视觉估算。截图仍是可见事实的最终依据。递归识别列表项、卡片和设置行中的稳定字段结构及独立操作控件，并用 relationships 表达关系。不要输出截图中不可见的节点。\\n${bounded}`;
}
