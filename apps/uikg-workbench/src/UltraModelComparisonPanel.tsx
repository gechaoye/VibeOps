...
import type { BBox, UltraModelElementMergeSelection, UltraModelMergeSource, RecognitionElementCandidate, RecognitionResult } from './types';

interface UltraModelComparisonPanelProps {
  imageUrl: string;
  modelAResult: RecognitionResult;
  modelBResult: RecognitionResult;
  applying: boolean;
  candidatePortalTarget: HTMLElement | null;
  onApply: (selections: UltraModelElementMergeSelection[]) => void;
}

type DiffStatus = 'same' | 'changed' | 'ultra-a-only' | 'ultra-b-only';
type MergeField = 'label' | 'elementType' | 'visualDescription' | 'interactive' | 'enabled' | 'state' | 'approximateRegion' | 'geometryKind' | 'geometryConfidence' | 'meaning' | 'dynamicContent' | 'riskSignals' | 'confidence' | 'actions';
type CandidateView = RecognitionElementCandidate & { actions?: Array<Record<string, unknown>> };

interface ComparisonRow {
  candidateKey: string;
  modelACandidateKey?: string;
  modelBCandidateKey?: string;
  modelA?: CandidateView;
  modelB?: CandidateView;
  status: DiffStatus;
...
const statusLabels: Record<DiffStatus, string> = {
  same: '一致',
  changed: '有差异',
  'ultra-a-only': '仅 Model A',
  'ultra-b-only': '仅 Model B',
};
...
function recognitionCandidate(element: RecognitionElementCandidate, result: RecognitionResult): CandidateView {
  return {
    ...element,
...
function buildComparisonRows(modelAResult: RecognitionResult, modelBResult: RecognitionResult) {
  const modelAByKey = new Map((modelAResult.elements || []).map((element) => [element.candidateKey, recognitionCandidate(element, modelAResult)]));
  const modelBByKey = new Map((modelBResult.elements || []).map((element) => [element.candidateKey, recognitionCandidate(element, modelBResult)]));
  const usedModelBKeys = new Set<string>();
  ...
  const rows = [...modelAByKey.entries()].map<ComparisonRow>(([modelACandidateKey, modelA]) => {
    let modelBCandidateKey = modelBByKey.has(modelACandidateKey) && !usedModelBKeys.has(modelACandidateKey) ? modelACandidateKey : undefined;
    if (!modelBCandidateKey) {
      const best = [...modelBByKey.entries()]
        .filter(([key]) => !usedModelBKeys.has(key) && !modelAByKey.has(key))
        .map(([key, modelB]) => ({ key, score: regionScore(modelA, modelB) }))
        .filter(({ score }) => score >= 0)
        .sort((left, right) => right.score - left.score)[0];
      modelBCandidateKey = best?.key;
    }
    if (modelBCandidateKey) usedModelBKeys.add(modelBCandidateKey);
    const modelB = modelBCandidateKey ? modelBByKey.get(modelBCandidateKey) : undefined;
    if (!modelB) return { candidateKey: modelACandidateKey, modelACandidateKey, modelA, status: 'ultra-a-only', changedFields: [] };
    const changedFields = mergeFields.map(({ key }) => key).filter((field) => stableValue(modelA[field]) !== stableValue(modelB[field]));
    return { candidateKey: modelACandidateKey, modelACandidateKey, modelBCandidateKey, modelA, modelB, status: changedFields.length ? 'changed' : 'same', changedFields };
  });
  for (const [modelBCandidateKey, modelB] of modelBByKey) {
    if (!usedModelBKeys.has(modelBCandidateKey)) rows.push({ candidateKey: modelBCandidateKey, modelBCandidateKey, modelB, status: 'ultra-b-only', changedFields: [] });
  }
  return rows;
}
...
function ResultCanvas({ source, imageUrl, rows, selectedKey, includedKeys, scrollRef, onScroll, onSelect, onToggleIncluded, onSelectAll, onClearAll }: {
  ...
  const title = source === 'modelA' ? 'Model A' : 'Model B';
...
export function UltraModelComparisonPanel({ imageUrl, modelAResult, modelBResult, applying, candidatePortalTarget, onApply }: UltraModelComparisonPanelProps) {
  const rows = useMemo(() => buildComparisonRows(modelAResult, modelBResult), [modelAResult, modelBResult]);
  const [selectedKeys, setSelectedKeys] = useState<Record<UltraModelMergeSource, string | null>>({ modelA: null, modelB: null });
  const [includedKeys, setIncludedKeys] = useState<Record<UltraModelMergeSource, Set<string>>>({ modelA: new Set(), modelB: new Set() });
  const modelAScrollRef = useRef<HTMLDivElement>(null);
  const modelBScrollRef = useRef<HTMLDivElement>(null);
  const syncingScrollRef = useRef(false);
  const statusCounts = useMemo(() => rows.reduce<Record<DiffStatus, number>>((counts, row) => {
    counts[row.status] += 1;
    return counts;
  }, { same: 0, changed: 0, 'ultra-a-only': 0, 'ultra-b-only': 0 }), [rows]);
  const modelCounts = {
    modelA: rows.filter((row) => row.modelA).length,
    modelB: rows.filter((row) => row.modelB).length,
  };
  const totalIncluded = includedKeys.modelA.size + includedKeys.modelB.size;

  useEffect(() => {
    setSelectedKeys({ modelA: null, modelB: null });
    setIncludedKeys({ modelA: new Set(), modelB: new Set() });
  }, [modelAResult, modelBResult]);

  const toggleIncluded = (source: UltraModelMergeSource, candidateKey: string, included: boolean) => {
  ...
  const selectCandidate = (source: UltraModelMergeSource, candidateKey: string) => setSelectedKeys((current) => ({ ...current, [source]: candidateKey }));
  const candidateKeys = (source: UltraModelMergeSource) => rows.flatMap((row) => row[source]?.candidateKey ? [row[source].candidateKey] : []);
  const selectAll = (source: UltraModelMergeSource) => setIncludedKeys((current) => ({ ...current, [source]: new Set(candidateKeys(source)) }));
  const clearAll = (source: UltraModelMergeSource) => setIncludedKeys((current) => ({ ...current, [source]: new Set() }));
  const syncScroll = (source: 'modelA' | 'modelB') => {
    if (syncingScrollRef.current) return;
    const origin = source === 'modelA' ? modelAScrollRef.current : modelBScrollRef.current;
    const target = source === 'modelA' ? modelBScrollRef.current : modelAScrollRef.current;
  ...
  const applySelection = () => {
    const selections: UltraModelElementMergeSelection[] = [
      ...[...includedKeys.modelA].map((candidateKey) => ({ candidateKey: `modelA:${candidateKey}`, modelACandidateKey: candidateKey, baseSource: 'modelA' as const, fieldSources: {} })),
      ...[...includedKeys.modelB].map((candidateKey) => ({ candidateKey: `modelB:${candidateKey}`, modelBCandidateKey: candidateKey, baseSource: 'modelB' as const, fieldSources: {} })),
    ];
    onApply(selections);
  };

  return (
    <section className="ultra-model-comparison" aria-label="双模型 识别结果">
      <header>
        <span><strong>双模型 识别画面</strong><small>A {modelCounts.modelA} · B {modelCounts.modelB}</small></span>
      </header>
      ...
      <div className="comparison-diff-summary" aria-label="差异汇总">
        ...
        <span className="ultra-a-only">仅 Model A {statusCounts['ultra-a-only']}</span>
        <span className="ultra-b-only">仅 Model B {statusCounts['ultra-b-only']}</span>
      </div>

      <div className="comparison-visual-results">
        <ResultCanvas source="modelA" imageUrl={imageUrl} rows={rows} selectedKey={selectedKeys.modelA} includedKeys={includedKeys.modelA} scrollRef={modelAScrollRef} onScroll={() => syncScroll('modelA')} onSelect={(key) => selectCandidate('modelA', key)} onToggleIncluded={(key, included) => toggleIncluded('modelA', key, included)} onSelectAll={() => selectAll('modelA')} onClearAll={() => clearAll('modelA')} />
        <ResultCanvas source="modelB" imageUrl={imageUrl} rows={rows} selectedKey={selectedKeys.modelB} includedKeys={includedKeys.modelB} scrollRef={modelBScrollRef} onScroll={() => syncScroll('modelB')} onSelect={(key) => selectCandidate('modelB', key)} onToggleIncluded={(key, included) => toggleIncluded('modelB', key, included)} onSelectAll={() => selectAll('modelB')} onClearAll={() => clearAll('modelB')} />
      </div>
      ...
      {candidatePortalTarget && createPortal(
        <section className="comparison-element-list" aria-label="候选元素选择">
          ...
          <div className="comparison-candidate-groups">
            <CandidateGroup source="modelA" rows={rows} selectedKey={selectedKeys.modelA} includedKeys={includedKeys.modelA} onSelect={(key) => selectCandidate('modelA', key)} onToggleIncluded={(key, included) => toggleIncluded('modelA', key, included)} onSelectAll={() => selectAll('modelA')} onClearAll={() => clearAll('modelA')} />
            <CandidateGroup source="modelB" rows={rows} selectedKey={selectedKeys.modelB} includedKeys={includedKeys.modelB} onSelect={(key) => selectCandidate('modelB', key)} onToggleIncluded={(key, included) => toggleIncluded('modelB', key, included)} onSelectAll={() => selectAll('modelB')} onClearAll={() => clearAll('modelB')} />
          </div>
        </section>,
