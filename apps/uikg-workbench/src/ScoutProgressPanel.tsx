...
export function ScoutProgressPanel({ activity, modelName, reviewerModel, autoMode, onCancel, onRetry, onClose, sessions, comparison, applyingComparison = false, onApplyComparison }: ScoutProgressPanelProps) {
  ...
  const historySession = activity.phase === 'history' ? sessions[0] : undefined;
  const inReviewerStage = reviewing || historySession?.kind === 'review';
  const scoutStepDone = inReviewerStage || activity.phase === 'review-compare' || activity.phase === 'complete' || historySession?.kind === 'scout' && historySession.status === 'completed';
  const reviewerStepDone = activity.phase === 'review-compare' || activity.phase === 'complete' || historySession?.kind === 'review' && historySession.status === 'completed';
  const scoutStepState = scoutStepDone ? 'done' : !inReviewerStage && activity.status === 'error' ? 'failed' : !inReviewerStage && (activity.status === 'cancelled' || activity.status === 'paused') ? 'paused' : !inReviewerStage && active ? 'active' : '';
  const reviewerStepState = reviewerStepDone ? 'done' : activity.phase === 'review-error' || historySession?.kind === 'review' && historySession.status === 'failed' ? 'failed' : activity.phase === 'review-cancelled' || activity.phase === 'review-paused' || historySession?.kind === 'review' && historySession.status === 'cancelled' ? 'paused' : inReviewerStage && active ? 'active' : '';
  const confirmationStepState = activity.phase === 'complete' ? 'done' : activity.phase === 'review-compare' ? 'active' : '';
      ...
      <section className="scout-progress-panel" role="dialog" aria-modal="true" aria-label={autoMode ? 'Auto 双模型分析' : 'Scout 实时分析'}>
        ...
        {autoMode && <div className="auto-review-pipeline" aria-label="Auto 审核流程">
          <div className="auto-review-pipeline-track">
            <span className={`pipeline-step ${scoutStepState}`}><i><ScanSearch size={16} /></i><span><small>第一步</small><strong>Scout 识别</strong><em>{modelName || '未配置'}</em></span></span>
            <span className={`pipeline-step ${reviewerStepState}`}><i><Bot size={16} /></i><span><small>第二步</small><strong>Reviewer 重识别</strong><em>{reviewerModel || '未配置'}</em></span></span>
            <span className={`pipeline-step ${confirmationStepState}`}><i><UserCheck size={16} /></i><span><small>第三步</small><strong>人工确认</strong><em>合并并审核</em></span></span>
          </div>
        </div>}
